...
<?php                                                            //我发表的评论
	...
	session_start();
   	$phone = $_SESSION['phone'];
	$sql = mysql_query("SELECT * FROM comment WHERE phone = $phone "); //$phone是评论者的账号
	while($row = mysql_fetch_array($sql)){
	$articleID = $row['ArticleID'];//ArticleID是文章ID

	$commentID = $row['CommentPhone'];//作者ID
	$sql2 = mysql_query("SELECT * FROM article WHERE Id = $articleID "); //Id是文章ID
	$row2 = mysql_fetch_array($sql2);
	$sql3 = mysql_query("SELECT * FROM register WHERE phone = $phone");
...
<table>
	<tr>
		...
		<td><font color="#999"><a href="deleteReview.php?id=<?php echo $row['Id'];?>"><label style=" width:150px;display:block;text-decoration:underline;">删除</label></a></font></td>
	</tr>
</table>
<?php	
	}
